...
with open("poker-hand-training-true.data") as pokerFile:
    dataReader = csv.reader(pokerFile)
    data = list(list(int(elem) for elem in row) for row in dataReader)

# defined a new primitive set for strongly typed GP
pset = gp.PrimitiveSetTyped("MAIN", itertools.repeat(int, 10), bool, "IN")
...
def evalSpambase(individual):
    ...
    training_sample = random.sample(data, 400)
    # Evaluate the sum of correctly identified hand
    result = sum(bool(func(*hand[:10])) is bool(hand[10]) for hand in training_sample)
    return result,
